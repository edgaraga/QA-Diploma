...
public class PaymentOnCreditTest {
    ...
    void shouldApproveFirstCardWithDoubleSurnameOfTheOwner() {
        ...
        creditPage.messageAboutSuccessfulPayment(); //сообщение которое должно быть
    ...
    void shouldLessThan16DigitsInTheCard() { //следует ввести меньше 16 цифр в карте
        ...
        val cvc = DataHelper.getValidCvc();
        creditPage.fillOutLine(cardNumber, month, year, owner, cvc);
        creditPage.messageAboutIncorrectDataFormat(); //сообщение которое должно быть
    }
    ...
    void should16ZerosInTheCard() { //следует ввести номер карты, содержащий 16 нулей
        ...
        val cvc = DataHelper.getValidCvc();
        creditPage.fillOutLine(cardNumber, month, year, owner, cvc);
        creditPage.messageAboutUnsuccessfulPaymentRefused(); //сообщение которое должно быть
    }
    ...
    void shouldMonthNumberMore12() { //ввод месяца 13
        ...
        val cvc = DataHelper.getValidCvc();
        creditPage.fillOutLine(cardNumber, month, year, owner, cvc);
        creditPage.messageAboutIncorrectCardExpirationDate(); //сообщение которое должно быть
    }
    ...
    void shouldYearFieldPrevious() { //предыдущий год
        ...
        val cvc = DataHelper.getValidCvc();
        creditPage.fillOutLine(cardNumber, month, year, owner, cvc);
        creditPage.messageAboutCardExpiration(); //сообщение которое должно быть
    }

    @Description
    //2.12_Негативный тест в поле "Год" ввести год на 6 лет больше текущего года
    @Test
    void shouldYearMoreThan5YearsOfTheCurrentYear() { //в поле "Год" ввести год на 6 лет больше текущего года
        val cardNumber = DataHelper.getFirstCardNumber();
        val month = DataHelper.getValidMonth(); //
    ...
    void shouldOwnerFieldEmpty() { //поле "Год" оставить пустым
        ...
        val cvc = DataHelper.getValidCvc();
        creditPage.fillOutLine(cardNumber, month, year, owner, cvc);
        creditPage.messageAboutTheMandatoryFillingInOfTheField(); //сообщение которое должно быть
    }
